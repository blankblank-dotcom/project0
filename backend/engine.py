...
class InferenceEngine:
    ...
    def _run_inference(self, image: Image.Image, prompt: str, max_new_tokens: int, temperature: float) -> EngineResult:
        ...
        """Run actual inference with the provided image and prompt."""
        ...
        try:
            ...
            # Convert image to numpy array for the VLMPipeline
            ...
            # Generate with VLMPipeline — pass BOTH image and the real prompt
            ...
            # Convert numpy array → openvino.Tensor (what VLMPipeline expects)
            ov_image = OVTensor(image_array)

            # Build GenerationConfig
            gen_config = ov_genai.GenerationConfig()
            gen_config.max_new_tokens = max_new_tokens

            # Generate: pass prompt string + images as list[OVTensor]
            raw_output = self._pipe.generate(
                prompt,
                images=[ov_image],
                generation_config=gen_config,
            )

            # VLMPipeline returns VLMDecodedResults — extract text
            if hasattr(raw_output, 'texts') and raw_output.texts:
                raw_text = raw_output.texts[0].strip()
            ...
            log.info(f"📄 Raw output preview: {raw_text[:200]!r}")
    ...
    def extract_structured_data(self, image: Image.Image, document_type: str = "Unknown") -> EngineResult:
        ...
        extraction_prompt = f"""
Extract all text from this {document_type}. Read and write:

1. All visible text exactly as shown
2. All names, emails, phone numbers
3. All dates, addresses, companies
4. All headers and section titles
5. All numbers and codes
...
The percentages must add up to 100%. Be confident in your primary choice.
    ...
    # SINGLE-PASS COMBINED CLASSIFY + EXTRACT

    # -----------------------------------------------------------------------

    def classify_and_extract(self, image: Image.Image) -> EngineResult:

        """Classify the document AND extract all fields in a single inference pass.



        This replaces the old two-step flow:

          classify_document_with_confidence() → 1 pass

          extract_structured_data()           → 2 passes

        …with a single prompt that asks for both at once, cutting total

        inference time roughly in half.



        The combined prompt produces:
        ...
        Returns:
            ...
            EngineResult with document_type, confidence, and objects populated.
        ...
        log.info("⚡ Starting single-pass classify+extract...")



        combined_prompt = """\
Analyze this image carefully. Reply in EXACTLY this format (no extra text before or after):
...
IMPORTANT:
- Replace "Resume" on the first line with the actual document type.
- Replace "92" on the second line with your confidence score (a plain integer from 0 to 100).
- After the --- separator, list EVERY piece of information as separate Field: Value lines.
- DOCUMENT_TYPE must be one of: Resume, CV, Cover Letter, Invoice, Receipt, Purchase Order,
  Quotation, Contract, Agreement, Business Letter, Report, Proposal, Payslip, Bank Statement,
  ID Card, Passport, Driving License, Visa, Certificate, Affidavit, Court Order, Patent,
  Academic Transcript, Degree, Exam Paper, Research Paper, Prescription, Lab Report,
  Medical Record, Insurance Claim, Chart, Diagram, Table, Spreadsheet, Screenshot,
  Code Snippet, Email, Photo, Illustration, Comic, Meme, Advertisement, Map, Other
- Be exhaustive. Each skill, job, line item, or detail = its own line.
- Skip only genuinely absent fields. No explanations, no markdown.
"""
        ...
        result = self.process_document(
            ...
            enable_pii_detection=False,  # PII is a separate explicit step
            ...
            if not in_fields:
                ...
                if low.startswith("document_type:"):
                    ...
                    try:
                        ...
                        confidence = float(stripped.split(":", 1)[1].strip().rstrip("%"))
                ...
                # Collect field lines (everything after ---)
        ...
        # ---------- if model didn't use the separator, heuristically split ----------

        if not in_fields:

            # Try to extract the type embedded in the raw text

            doc_type_pattern = re.compile(

                r'\b(Resume|CV|Invoice|Receipt|ID\s*Card|Contract|Form|Letter|Report|Payslip|Passport|Certificate|Transcript|Prescription|Other)\b',
            ...
            m = doc_type_pattern.search(result.raw_text)

            if m:

                doc_type = m.group(1).strip()

                # Model skipped the CONFIDENCE line — assume high confidence

                if confidence == 0.0:

                    confidence = 90.0

            # treat everything as field lines

            field_lines = [

                l.strip() for l in result.raw_text.splitlines()
        ...
        # ---------- build GroundedObject list from field lines ----------
        ...
        for line in field_lines:
            ...
            fname = self._clean_field_name(parts[0])

            fval = parts[1].strip()

            if not fval or len(fval) < 2 or fval.lower() in ("n/a", "none", "not found", "unknown"):

                continue

            # Skip header lines that leaked through

            if fname.lower() in ("document_type", "confidence"):

                continue

            objects.append(GroundedObject(label=fname, value=fval, box_2d=[0.0, 0.0, 0.0, 0.0]))



        # fall back to raw content if nothing was parsed

        if not objects and result.raw_text:

            objects.append(GroundedObject(

                label="Extracted Content",

                value=result.raw_text[:500] + ("..." if len(result.raw_text) > 500 else ""),
        ...
        log.info(
            ...
            f"✅ Single-pass done in {result.processing_time:.1f}s — "

            f"type={doc_type} ({confidence}%), fields={len(objects)}"
    ...
    def process_document_with_grounding(self, image: Image.Image, prompt: Optional[str] = None, document_type: str = "Unknown") -> EngineResult:
        ...
        """Process document with optional prompt; forwards to extract_structured_data."""

        if prompt:

            # Caller supplied a specific prompt — honour it directly (single pass)
        ...
        # No prompt supplied — fall back to structured extraction
    ...
    def detect_pii(self, image: Image.Image) -> EngineResult:
        ...
        """Detect PII in a single inference pass.



        Replaces the old 3-pass flow (classify → raw-OCR → field-parse) with

        a focused 512-token prompt that asks the model to output ONLY PII fields

        it can actually see, using a simple Field: Value format that is reliably

        parseable without special tokens.
        ...
        log.info("🔒 Starting single-pass PII detection...")



        pii_prompt = """\

List ONLY these privacy-sensitive fields if they are visible in the document.
...
If a field is NOT visible, skip that line entirely.

Do NOT output any other fields, explanations, or markdown.
...
"""
        ...
        result = self.process_document(
            ...
            max_new_tokens=512,   # PII fields are short — 512 is plenty
        ...
        if result.error:
            ...
            log.error(f"❌ PII detection failed: {result.error}")

            return result



        # Parse Field: Value lines into GroundedObjects

        objects: list[GroundedObject] = []

        skip_values = {"n/a", "none", "not found", "unknown", "<value>", ""}
        ...
        for line in result.raw_text.splitlines():
            ...
            if fval.lower() in skip_values or len(fval) < 2:
        ...
        log.info(
            ...
            f"✅ PII detection done in {result.processing_time:.1f}s — "

            f"{len(objects)} PII field(s) found"
    ...
    def _parse_grounding(raw_text: str) -> list[GroundedObject]:
        ...
        try:
            ...
            log.debug(f"Error during initial text parsing: {e}")
            ...
            try:
                ...
                for match in coord_regex.finditer(raw_text):
                    try:
                        ...
                        log.debug(f"Unexpected error parsing naked coords: {e}")
                        continue
            except Exception as e:
                log.debug(f"Failed to initialize coordinate parsing: {e}")
